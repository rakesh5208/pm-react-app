...
const ColumnCustomizerV1 = <T,>({
  ...
  const copiedFrozenColumns = useMemo(
    () => [...frozenColumns],
    [frozenColumns]
  );
  const copiedOtherSelectedColumns = useMemo(
    () => [...otherSelectedColumns],
    [otherSelectedColumns]
  );

  const [allCustomizerFields, setAllCustomizerFields] = useState<
    Array<ColumnCustomizerField>
  >([]);

  const [frozenColumnList, setFrozenColumnList] = useState(copiedFrozenColumns);
  const [otherSelectedColumnsList, setOtherSelectedColumnsList] = useState(
    copiedOtherSelectedColumns
  );

  const frozenColumnIds = useMemo(
    () => frozenColumnList.map((col) => col.id),
    [frozenColumnList]
  );
  const otherSelectedColumnIds = useMemo(
    () => otherSelectedColumnsList.map((col) => col.id),
    [otherSelectedColumnsList]
  );
  ...
  useEffect(() => {
    if (isOpenPopover) {
      ...
      const allOtherFields = copiedAllColumns
        .filter(
          (col) =>
            !(
              frozenColumnIds.includes(col.id) ||
              otherSelectedColumnIds.includes(col.id)
            )
        )
        .map((col) => {
          return {
      ...
      setAllCustomizerFields([
        ...frozenFieldFromAllColumns,
        ...otherSelectedFieldFromAllColumns,
        ...allOtherFields,
      ]);
    }
  }, [isOpenPopover]);
  ...
  const onColSelectStateChange = useCallback((col: ColumnCustomizerField) => {
    ...
    if (!col.isSelected) {
      setOtherSelectedColumnsList((prev) => [
        ...prev,
        allColumnsAsMapWithId[col.id],
      ]);
    } else {
      setOtherSelectedColumnsList((prev) =>
        prev.filter((field) => field.id !== col.id)
      );
    }
  }, []);
  ...
  const onFrozenStateChange = useCallback((col: ColumnCustomizerField) => {
    if (col.isColumnFreezed) {
      ...
      setFrozenColumnList((prev) =>
        prev.filter((frozenColumn) => frozenColumn.id !== col.id)
      );
  ...
  const handleUpdate = () => {
    console.log(
      "Selected fields with state =>",
      frozenColumnList,
      otherSelectedColumnsList
    );
    onUpdate({
      frozenColumns: [...frozenColumnList],
      otherSelectedColumns: [...otherSelectedColumnsList],
    });
    setIsOpenPopover(false);
  };
              ...
              <div className="flex flex-col w-1/2 border-r border-primary-border py-2">
                ...
                <ul className="flex-1 overflow-y-auto px-4">
                  {allCustomizerFields.map((col) => {
                    return (
                      <li
                        key={`all-customizer-field-${col.id}`}
                        className="py-1 mt-1 text-prop"
                      >
                        <div className="flex items-top gap-2">
                          <Checkbox
                            ...
                            disabled={col.isColumnFreezed}
                            onCheckedChange={() => onColSelectStateChange(col)}
                          />
                          <label
                            className={`align-top wrap-anywhere all-customizer-field-${
                              col.id
                            } ${
                              col.isColumnFreezed
                                ? "cursor-not-allowed"
              ...
              <div className="w-1/2 flex flex-col  py-2">
                <div className="overflow-y-auto flex-1 pr-4 pb-2">
                  ...
                  <SelectedColumns
                    ...
                    name="locked_columns"
                    columns={frozenFieldFromAllColumns}
                    onFreezeStateChange={onFrozenStateChange}
                    onRemoveColumn={onRemoveColumn}
                    onDropAndSort = { (sortedColumns) => (console.log("Locked Columns >> sorted columns >>", sortedColumns))}
                  />

                  {/* reorderable columns */}
                  <SelectedColumns
                    name="removable_columns"
                    title="Removable Columns"
                    columns={otherSelectedFieldFromAllColumns}
                    onFreezeStateChange={onFrozenStateChange}
                    onRemoveColumn={onRemoveColumn}
                    onDropAndSort = { (sortedColumns) => (console.log("Removable Columns >> sorted columns >>", sortedColumns))}
                  />
                </div>
              </div>
            </div>
